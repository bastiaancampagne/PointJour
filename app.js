...
function finishSession(){state.sessionReady=true;state.scope=state.chosen.private&&state.chosen.work?'all':(state.chosen.private?'private':'work');state.page='home';persistSession();render()}
function initGoogle(){
...
function webPage(){return topItems('🌐 Recherche Web','PointJour combine votre thème, vos sous-thèmes et vos sources actives.','assets/emu-search.png')+`<div class="card"><div class="field"><label>Mot-clé complémentaire</label><input id="globalq" placeholder="Ex. nouveautés, guide, barème, recette…"></div><div class="field"><label>Choisir une veille</label><select id="globalwatch"><option value="">Recherche générale</option>${state.watches.filter(w=>w.active).map(w=>`<option>${esc(w.name)}</option>`).join('')}</select></div><button class="primary" id="globalsearch">Rechercher sur le Web</button></div>${state.watches.filter(w=>w.active).map(w=>`<div class="card source"><div><h3>🔎 ${esc(w.name)}</h3><span class="meta">${w.subs.length} sous-thèmes · ${sourceDomainsFor(w.name).length} source(s) active(s)</span></div><button class="secondary" data-search-watch="${state.watches.indexOf(w)}">Lancer</button></div>`).join('')}`}
function archives(){const themes=['all',...new Set(state.archive.map(x=>x.theme).filter(Boolean))],items=state.archive.filter(x=>(state.scope==='all'||x.scope==='all'||x.scope===state.scope)&&(state.archiveTheme==='all'||x.theme===state.archiveTheme));return topItems('▣ Archives','Un vrai historique : chaque résultat peut être rouvert.','assets/emu-breakfast.png')+scopeTabs()+`<div class="tabs archive-themes">${themes.map(t=>`<button class="${state.archiveTheme===t?'primary':'secondary'}" data-archive-theme="${esc(t)}">${esc(t==='all'?'Tous les thèmes':t)}</button>`).join('')}</div>${items.map(x=>`<a class="item archive-item" href="${esc(x.url)}" target="_blank" rel="noopener"><span class="badge">${esc(x.theme||'Général')}</span><h3>${esc(x.title||'Résultat')}</h3><p>${esc(x.source||'Web')}</p><span class="meta">${esc(new Date(x.date).toLocaleString('fr-FR',{dateStyle:'medium',timeStyle:'short'}))}</span><strong>Ouvrir la source ↗</strong></a>`).join('')||'<div class="empty">Aucun résultat archivé pour ce filtre.</div>'}`}
...
function bind(){
 ...
 if($('#continueNoGoogle'))$('#continueNoGoogle').onclick=async()=>{state.chosen={private:false,work:false};state.loading=true;persistSession();render();try{await loadNews();state.error=''}catch(e){state.error=e.message}state.loading=false;state.sessionReady=true;state.page='home';persistSession();render()};
