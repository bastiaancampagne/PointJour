...
function filterNews(w){const terms=w.subs.map(x=>x.toLowerCase()),theme=(w.name||'').toLowerCase();return state.news.filter(a=>{if((a.theme||'').toLowerCase()!==theme)return false;const t=[a.title,a.summary,...(a.topics||[])].join(' ').toLowerCase();return !terms.length||terms.some(x=>t.includes(x))})}
