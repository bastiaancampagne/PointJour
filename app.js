...
const state={page:'signin',token:null,email:'',messages:[],events:[],news:[],watches:load('pj_watches',defaults),archive:load('pj_archive',[]),selected:0,tab:'today',error:'',loading:false};
...
function setPage(p){state.page=p;$('#nav')?.classList.toggle('hidden',!state.token);document.querySelectorAll('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===p));render();scrollTo(0,0)}

function scene(title,caption,img='assets/emu-breakfast.png'){
 return `<section class="scene"><img src="${img}" alt="Émeu PointJour"><div class="scene-copy"><h1>${title}</h1><p>${caption}</p><span>☕ Un bon café, les bonnes infos, une journée bien organisée !</span></div></section>`;
}
...
function initGoogle(){
 ...
 tokenClient=google.accounts.oauth2.initTokenClient({client_id:CFG.GOOGLE_CLIENT_ID,scope:SCOPES,callback:async r=>{
  ...
  state.token=r.access_token;await refreshAll();setPage('home');
 }});return true;
}
function connect(){
...
async function api(u){const r=await fetch(u,{headers:{Authorization:`Bearer ${state.token}`}});if(!r.ok)throw Error('Google API '+r.status);return r.json()}
function gh(h,n){return(h||[]).find(x=>x.name?.toLowerCase()===n.toLowerCase())?.value||''}
async function loadGoogle(){
 const p=await api('https://gmail.googleapis.com/gmail/v1/users/me/profile');state.email=p.emailAddress||'';
 ...
 state.messages=(await Promise.all((refs.messages||[]).map(async x=>{try{
  const m=await api(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${x.id}?format=metadata&metadataHeaders=From&metadataHeaders=Subject&metadataHeaders=Date`),h=m.payload?.headers||[];
  return{id:m.id,sender:gh(h,'From'),subject:gh(h,'Subject')||'(Sans objet)',date:gh(h,'Date'),snippet:m.snippet||'',time:+m.internalDate||0}
 }catch{return null}}))).filter(Boolean).sort((a,b)=>b.time-a.time);
 ...
 state.events=(ev.items||[]).map(e=>({id:e.id,title:e.summary||'Sans titre',start:e.start?.dateTime||e.start?.date||'',link:e.htmlLink||'',location:e.location||''}));
}
...
async function refreshAll(){state.loading=true;state.error='';render();try{await Promise.all([loadGoogle(),loadNews()]);snapshot()}catch(e){state.error=e.message}state.loading=false;render()}
function snapshot(){const d=new Date().toISOString().slice(0,10),entry={date:d,mails:state.messages.length,events:state.events.length,news:state.news.slice(0,20)};state.archive=[entry,...state.archive.filter(x=>x.date!==d)].slice(0,60);save('pj_archive',state.archive)}

function loadingScreen(){return `<section class="loading-screen"><img src="assets/emu-loading.png" alt="Émeu avec café"><h1>☀️ PointJour</h1><p>Préparation de votre brief…</p><div class="progress"><span></span></div><small>Analyse de Gmail, Agenda et de vos veilles en cours.</small></section>`}
function signin(){return `<section class="signin-scene"><img src="assets/emu-breakfast.png" alt="Émeu au petit déjeuner"><div><img class="signin-logo" src="icon-192.png" alt="PointJour"><h1>PointJour</h1><h3>Votre journée, l’essentiel en un coup d’œil</h3><p>Gmail, Google Agenda et vos veilles personnalisées, réunis autour d’un café.</p>${state.error?`<div class="notice error">${esc(state.error)}</div>`:''}<button class="primary" id="connect">Se connecter avec Google</button><p class="tag">Lecture seule : Gmail + Google Agenda.</p>${!configured()?'<div class="notice">Renseignez le client OAuth Web dans <b>config.js</b>.</div>':''}</div></section>`}
function home(){const active=state.watches.filter(w=>w.active);return topItems('☀️ PointJour','Bonjour ! Installez-vous : l’émeu a préparé votre point du jour.')+`<div class="welcome card"><b>Bonjour${state.email?', '+esc(state.email.split('@')[0]):''} !</b><p>Voici l’essentiel de votre journée.</p>${state.error?`<div class="notice error">${esc(state.error)}</div>`:''}</div><div class="grid"><button class="tile" data-go="mail">📧<strong>Gmail</strong><span>${state.messages.length} messages</span></button><button class="tile" data-go="calendar">📅<strong>Agenda</strong><span>${state.events.length} événements / 8 jours</span></button>${active.map(w=>`<button class="tile" data-watch="${state.watches.indexOf(w)}">🔎<strong>${esc(w.name)}</strong><span>${w.subs.length} sous-thèmes</span></button>`).join('')}</div><div class="card quick"><button class="primary" id="refresh">↻ Actualiser</button><button class="secondary" data-go="brief">✓ Accéder à mon brief</button></div>`}
function mail(){return topItems('📧 Courriels','Les nouveaux messages utiles, sans promotions ni spam.','assets/emu-brief.png')+`<div class="card hint">Promotions, réseaux sociaux, forums et spam sont exclus.</div>${state.messages.map(m=>`<div class="item" data-mail="${m.id}"><h3>${esc(m.subject)}</h3><b>${esc(m.sender)}</b><p>${esc(m.snippet)}</p><span class="meta">${esc(m.date)}</span></div>`).join('')||'<div class="empty">Aucun message.</div>'}`}
function calendar(){return topItems('📅 Agenda','Aujourd’hui et les sept prochains jours, pour garder le rythme.','assets/emu-brief.png')+`<div class="card"><b>Aujourd’hui + 7 jours</b></div>${state.events.map(e=>`<div class="item" data-event="${esc(e.link)}"><h3>${esc(e.title)}</h3><p>${esc(new Date(e.start.length===10?e.start+'T00:00:00':e.start).toLocaleString('fr-FR',{dateStyle:'medium',timeStyle:e.start.length===10?undefined:'short'}))}</p>${e.location?`<span class="meta">📍 ${esc(e.location)}</span>`:''}</div>`).join('')||'<div class="empty">Aucun événement.</div>'}`}
function brief(){const active=state.watches.filter(w=>w.active);return topItems('✓ Brief du jour','L’essentiel d’abord, les détails ensuite.','assets/emu-brief.png')+`<div class="brief-grid"><div class="card"><h3>📧 Gmail</h3><p>${state.messages.length} message(s) récent(s).</p><button class="linkbtn" data-go="mail">Voir les messages →</button></div><div class="card"><h3>📅 Agenda</h3><p>${state.events.length} événement(s) sur 8 jours.</p><button class="linkbtn" data-go="calendar">Voir l’agenda →</button></div>${active.map(w=>{const paie=w.name.toLowerCase()==='paie',n=paie?filterNews(w).length:null;return `<div class="card"><h3>🔎 ${esc(w.name)}</h3><p>${paie?n+' résultat(s) collecté(s), les plus récents en premier.':'Recherche ciblée prête sur '+w.subs.length+' sous-thème'+(w.subs.length>1?'s':'')+'.'}</p>${paie?`<button class="linkbtn" data-watch="${state.watches.indexOf(w)}">Voir les résultats →</button>`:`<button class="linkbtn" data-search-watch="${state.watches.indexOf(w)}">Lancer la recherche →</button>`}</div>`}).join('')}</div>`}
...
function watchPage(){const w=state.watches[state.selected];if(!w)return watches();const paie=w.name.toLowerCase()==='paie';const list=paie?filterNews(w):[];const today=new Date().toISOString().slice(0,10);const shown=state.tab==='today'?list.filter(a=>String(a.publishedAt||a.discoveredAt||'').slice(0,10)===today):list.filter(a=>String(a.publishedAt||a.discoveredAt||'').slice(0,10)!==today);const domains=TRUSTED[w.name]||[];return topItems('🔎 Veille — '+esc(w.name),'Votre veille personnalisée, autour d’un café.','assets/emu-watches.png')+`<div class="tabs"><button class="${state.tab==='today'?'primary':'secondary'}" data-tab="today">Aujourd’hui</button><button class="${state.tab==='archive'?'primary':'secondary'}" data-tab="archive">Archives</button></div>${paie?(shown.map(article).join('')||'<div class="empty">Aucun résultat dans cette rubrique.</div>'):`<div class="card"><h3>Recherche sur sources fiables</h3><p>${w.subs.map(x=>`<span class="badge">${esc(x)}</span>`).join(' ')}</p><div class="row">${w.subs.slice(0,20).map(s=>`<button class="secondary" data-search="${esc(s)}">🔎 ${esc(s)}</button>`).join('')}</div>${domains.length?`<p class="meta">Domaines privilégiés : ${esc(domains.join(', '))}</p>`:''}</div>`}`}
...
function settings(){return topItems('☰ Paramètres','Une application bien réglée pour une journée plus sereine.','assets/emu-breakfast.png')+`<div class="settings-grid"><div class="card"><h3>Compte Google</h3><p>${esc(state.email)}</p><p class="tag">Gmail et Agenda sont en lecture seule.</p></div><div class="card"><h3>Veilles personnalisées</h3><p>1 à 3 veilles, jusqu’à 20 sous-thèmes chacune.</p><button class="secondary" data-go="watches">Gérer mes veilles</button></div><div class="card"><h3>Sources et recherche</h3><p>Accès aux sources fiables et aux recherches Web ciblées.</p><div class="row"><button class="secondary" data-go="sources">Sources</button><button class="secondary" data-go="search">Recherche</button></div></div><div class="card"><h3>Sécurité</h3><p>Aucune clé secrète de moteur de recherche n’est stockée dans la PWA.</p></div></div><button class="danger" id="logout">Se déconnecter</button>`}
function morePage(){return topItems('☰ Plus','Tout le reste à portée de bec.','assets/emu-breakfast.png')+`<div class="menu-list"><button data-go="search">⌕ <span><b>Recherche</b><small>Trouver rapidement une information</small></span>›</button><button data-go="sources">☷ <span><b>Sources officielles</b><small>Accéder aux sites fiables</small></span>›</button><button data-go="experts">👥 <span><b>Experts / Web</b><small>Recherches publiques ciblées</small></span>›</button><button data-go="settings">⚙️ <span><b>Paramètres</b><small>Connexion, veilles et sécurité</small></span>›</button></div>`}

function render(){
 let h='';
 if(state.loading&&state.token)h=loadingScreen();
 else if(!state.token)h=signin();
 ...
 else if(state.page==='search')h=searchPage();
 else if(state.page==='sources')h=sourcesPage();
 else if(state.page==='experts')h=expertsPage();
 else if(state.page==='archives')h=archives();
 else if(state.page==='settings')h=settings();
 else h=morePage();
 $('#app').innerHTML=h;bind();
}
function bind(){
 document.querySelectorAll('[data-go]').forEach(x=>x.onclick=()=>setPage(x.dataset.go));
 $('#connect')&&($('#connect').onclick=connect);$('#refresh')&&($('#refresh').onclick=refreshAll);$('#logout')&&($('#logout').onclick=()=>{state.token=null;setPage('signin')});
 ...
 if($('#wsave'))$('#wsave').onclick=()=>{const name=$('#wname').value.trim()||'Veille',subs=$('#wsubs').value.split('\n').map(x=>x.trim()).filter(Boolean).slice(0,20);state.watches[state.selected]={name,active:$('#wactive').checked,subs};save('pj_watches',state.watches);toast('Veille enregistrée');setPage('watches')};
