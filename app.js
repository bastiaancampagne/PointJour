...
async function loadNews(){let j=null;for(const u of [CFG.NEWS_URL,'data/veille.json']){if(!u)continue;try{const r=await fetch(u+(u.includes('?')?'&':'?')+'t='+Date.now(),{cache:'no-store'});if(r.ok){j=await r.json();break}}catch{}}state.news=(Array.isArray(j?.items)?j.items:[]).sort((a,b)=>new Date(b.publishedAt||b.discoveredAt||0)-new Date(a.publishedAt||a.discoveredAt||0));state.newsUpdatedAt=j?.generated_at||null}
...
function snapshot(){state.news.slice(0,30).forEach(a=>{if(!a?.url)return;const w=state.watches.find(x=>x.name.toLowerCase()===(a.theme||'').toLowerCase());addArchive({title:a.title,url:a.url,source:a.source||'Source',theme:a.theme||'Général',scope:w&&(w.spaces||[]).length===1?w.spaces[0]:'all'})})}
...
function watchPage(){
 const w=state.watches[state.selected],items=filterNews(w),{today,older,undated}=splitNews(items),updated=state.newsUpdatedAt?new Date(state.newsUpdatedAt).toLocaleString('fr-FR',{dateStyle:'medium',timeStyle:'short'}):null;
 let body;
 ...
 return topItems(`🔎 ${esc(w.name)}`,`${w.subs.length} sous-thèmes configurés · ${(w.spaces||[]).map(x=>labels[x]).join(' + ')}`)
  +`<div class="tabs"><button class="${state.tab==='today'?'primary':'secondary'}" data-tab="today">Aujourd’hui</button><button class="${state.tab==='archive'?'primary':'secondary'}" data-tab="archive">Archives</button></div>`
  +`<div class="card row"><button class="secondary" id="watchRefresh">↻ Actualiser les trouvailles</button>${updated?`<span class="meta">Dernière mise à jour : ${esc(updated)}</span>`:''}</div>`
  +body;
}
...
function bind(){
 ...
 document.querySelectorAll('[data-search-watch]').forEach(x=>x.onclick=()=>{const idx=+x.dataset.searchWatch;if(!state.watches[idx])return;state.selected=idx;state.tab='today';persistSession();setPage('watch')});
 if($('#watchRefresh'))$('#watchRefresh').onclick=async()=>{toast('Actualisation…');try{await loadNews();toast('Veille actualisée')}catch(e){toast('Impossible d’actualiser pour le moment')}render()};
...
window.addEventListener('load',async()=>{document.querySelectorAll('#nav button').forEach(b=>b.onclick=()=>setPage(b.dataset.page));setTimeout(initGoogle,700);
 if(state.sessionReady){
  state.loading=true;render();
  try{await Promise.all([...connectedKeys().map(loadGoogleAccount),loadNews()]);snapshot();state.error=''}catch(e){state.error=e.message}
